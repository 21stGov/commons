// SPDX-License-Identifier: MIT

import { Prose } from '@21stgov/commons-react'
import * as React from 'react'

export const title = 'Prose'

export default function Demo(): React.JSX.Element {
  return (
    <div className="flex flex-col gap-8">
      <Prose as="article">
        <h2>Applying for a building permit</h2>
        <p>
          Most residential projects — additions, decks, and major renovations — require a
          permit before work begins. Review the checklist below, then reach out to{' '}
          <a href="/contact">the permits office</a> if you have questions about your project.
        </p>
        <h3>What you will need</h3>
        <ul>
          <li>A completed application form</li>
          <li>Proof of property ownership</li>
          <li>
            Site plans that reference the <code>setback</code> requirements for your zoning
            district
          </li>
        </ul>
        <blockquote>
          <p>Permits typically process within 10 business days of a complete submission.</p>
        </blockquote>
        <p>You can check the status of an existing application with the permits API:</p>
        <pre>
          <code>{'GET /api/permits/status?id=1234'}</code>
        </pre>
        <hr />
        <p>
          Still have questions? Read the <a href="/faq">frequently asked questions</a> or
          contact the office directly.
        </p>
      </Prose>

      <Prose as="article" size="sm">
        <h2>Small size</h2>
        <p>The same article content, rendered at the compact reading measure and type scale.</p>
      </Prose>

      <div dir="rtl" lang="ar">
        <Prose as="article">
          <h2>طلب الحصول على تصريح بناء</h2>
          <p>
            تتطلب معظم المشاريع السكنية تصريحاً قبل بدء العمل. راجع القائمة أدناه وتواصل مع{' '}
            <a href="/contact">مكتب التصاريح</a> إذا كانت لديك أي أسئلة.
          </p>
        </Prose>
      </div>
    </div>
  )
}
