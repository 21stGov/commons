...
export interface BuildOptions {
  ...
  version: string;
  /**
   * Directory of generated `.cui-*` demo fragments
   * (`apps/html-playground/src/generated/demos`). When set, each item gets an
   * `html` field with its framework-agnostic markup for the non-React path.
   */
  demosDir?: string;
...
export function buildItem(
  ...
  // The framework-agnostic `.cui-*` markup for the non-React path — the rendered
  // demo fragment the docs show (generated by the CSS package's demo pipeline).
  // Attached when a demo exists so `get_component` serves correct HTML markup to
  // non-React agents alongside the React files.
  const demoPath = demosDir ? join(demosDir, `${fragment.name}.html`) : undefined;
  const html =
    demoPath && existsSync(demoPath) ? readFileSync(demoPath, "utf8").trim() : undefined;
...
export function buildRegistry(options: BuildOptions): BuildResult {
  ...
  const items = findFragmentPaths(options.reactDir).map((fragmentPath) =>
    buildItem(fragmentPath, options.reactDir, options.version, options.demosDir),
  );
