...
function toClasses(value: unknown): string[] {
...
async function captureComponent(entry: string): Promise<CapturedVariant[]> {
...
const classBase = (exportName: string): string =>
 ...
 */
function splitUtilities(classes: string[]): { self: string[]; descendants: Map<string, string[]> } {
  const self: string[] = []
 ...
 */
const isMarker = (cls: string): boolean =>
  cls === 'group' || cls === 'peer' || /^(group|peer)\//.test(cls)

function emitRule(selector: string, rawClasses: string[], out: string[]): void {
...
/** One component export -> its `.cui-*` rules. */
function emitVariant(cap: CapturedVariant, out: string[]): void {
  const sel = `.cui-${classBase(cap.exportName)}`
  const baseClasses = [...cap.base]
  const modifiers: Array<{ sel: string; classes: string[] }> = []
  ...
  for (const [group, values] of Object.entries(cap.variants)) {
    ...
    for (const [key, classes] of Object.entries(values)) {
      if (isBoolean) {
        if (key === 'true') {
          modifiers.push({ sel: `${sel}--${camelToKebab(group)}`, classes })
        } else if (String(def) === 'false') {
          baseClasses.push(...classes) // default off-state folds into the base rule
        } else {
          modifiers.push({ sel: `${sel}--${camelToKebab(group)}-false`, classes })
        }
      } else {
        modifiers.push({ sel: `${sel}--${camelToKebab(key)}`, classes })
      }
    }
  }
...
// --- main: capture all components, emit, compile ----------------------------
...
async function main(): Promise<void> {
  ...
  try {
    for (const name of names) {
      ...
      const out: string[] = []
      for (const cap of caps) emitVariant(cap, out)
  ...
  // 1. write the @apply source.
  const distDir = join(here, 'dist')
  mkdirSync(distDir, { recursive: true })
  const header = `/* SPDX-License-Identifier: MIT */\n/* GENERATED by packages/css/build.ts from the components' cva() definitions. Do not edit. */\n`
  const srcCss = `${header}\n@layer components {\n${rulesByComponent.join('\n\n')}\n}\n`
  writeFileSync(join(distDir, 'components.src.css'), srcCss)

  // 2. compile @apply -> token-based CSS via Tailwind (no preflight; core owns
  // the reset). Self-contained: consumers link tokens + this file, no Tailwind.
...
`
  ...
  console.log(`\ncaptured ${captured.length}/${names.length} components; skipped ${skipped.length} (no cva):`)
  console.log(`  ${skipped.map((s) => s.replace(/ \(no cva\)/, '')).join(', ')}`)
  console.log(`\nwrote dist/components.src.css and dist/commons.css`)
}
